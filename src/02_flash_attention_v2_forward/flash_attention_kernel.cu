    ...
    for (int block_idx = 0; block_idx < num_blocks; block_idx++) {
        ...
        for (int local_row = 0; local_row < rows_per_warp; local_row++) { // each row of Q in this warp
            ...
            if (global_row < global_row_end) {
                ...
                for (int col = 0; col < actual_Bc; col += 1) { // each column in K_smem
                    float dot = 0.0f;
                ...
                // __syncthreads();
                // if (tid == 0){
                //     printf("Warp_id\tlane_id\tThread\tRow\tS_row\tLocalRow;  S_row_size_per_thread=%d\n", S_row_size_per_thread);
                // }
                // for (int k = 0; k < S_row_size_per_thread; k++) {
                //     printf("%d \t %d \t %d \t %d \t %f \t %d\n", warp_id, lane_id, tid, k, S_row[k],local_row);

                // }
                // __syncthreads();
                /**
                * Example output for S_row when Bc=4, WARP_SIZE=2
                warpid\lane_id 0        1        0        1  \ local_row
                    0    1.000000 0.000000 0.000000 1.000000    0
                    0    0.000000 1.000000 0.000000 1.000000    1
                    1    0.000000 0.000000 1.000000 0.000000    0
                    1    1.000000 1.000000 0.000000 2.000000    1
                 */
                ...
                // Row 0: row_max=1.000000, row_sum=2.735759 
                // Row 1: row_max=1.000000, row_sum=2.735759
                // Row 2: row_max=1.000000, row_sum=2.103638
                // Row 3: row_max=2.000000, row_sum=1.871094


                // Compute exp and sum
                float row_sum = 0.0f;
                for (int col = 0; col < S_row_size_per_thread; col++) {
                    S_row[col] = expf(S_row[col] - m_new);
                    row_sum += S_row[col];
                }
                // Reduction to get row_sum across all threads in the warp
                ...
                // __syncthreads();
                // if(lane_id == 0){
                //     printf( "Row %d: row_sum=%f\n", rows_per_warp * warp_id + local_row, row_sum);
                // }
                // __syncthreads();
                ...
                // if(lane_id == 0){
                //     //print l
                //     printf( "Row %d: Updated l=%f, exp_diff=%f\n", rows_per_warp * warp_id + local_row, l[local_row], exp_diff);
                // }
                // __syncthreads();
                ...
                // Accumulate P @ V => O_acc[local_row]
                for (int V_col = 0; V_col < d; V_col += 1) { // each column in V_smem
                    float O_val = 0.0f;

                    for (int k = 0; k < S_row_size_per_thread; k += 1) {// iterate over each row in V_smem
                        int v_row = k * WARP_SIZE + lane_id;
                        O_val += S_row[k] * V_smem[v_row * d + V_col];
                        // if(tid <=1  && local_row ==0 ){
                        //     printf("Warp %d, lane_id = %d, Local Row %d, V_col %d, k %d: S_row=%f, V_smem=%f, Partial O_val=%f\n", warp_id, lane_id, local_row, V_col, k, S_row[k], V_smem[v_row * d + V_col], O_val);
                        // }
                    }
