...
// Simple forward pass for reference (needed to get O and L)
void naive_attention_forward(
    ...
    float* P = new float[seq_len * seq_len];
...
void test_simple_backward() {
    ...
    naive_attention_forward(h_Q, h_K, h_V, h_O, h_L, seq_len, head_dim, softmax_scale);
